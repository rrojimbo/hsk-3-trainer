...
### Cloudflare Pages (бесплатно, с внешними медиа)

Cloudflare Pages имеет лимит `25 MB` на один файл. Для больших аудио нужно вынести медиа
в отдельное хранилище (например Cloudflare R2) и подключить базовый URL через переменную среды.

Переменная:

- `VITE_MEDIA_BASE_URL=https://media.hsk3sprint.space`

После этого ссылки вида `/official/...` автоматически будут открываться как:
`https://media.hsk3sprint.space/official/...`.

Важно:

- крупные файлы из `public/official` уберите из репозитория, иначе деплой в Pages будет падать.
